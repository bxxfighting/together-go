...
> 获取token方法:  
> 手机上安装软件HttpCanary软件，在软件中点 设置 -> 其它选项中的安装平行空间  
> 在平行空间中打开微信，同时在HttpCanary软件中抓取平行空间的网络包。  
> 然后在平行空间中的微信访问捉妖雷达小程序，然后在HttpCanary中查看带publich5字样的链接的返回值  
> 将里面的token和openid设置上即可使用，一般可以使用两个小时，过期后，重新获取  


# 互动交流
请先Star后加群，进群申请输入：净化游戏环境从我做起，坚决不使用外挂等破坏游戏平衡。并提供自己的github名称，确认是否点了Star  
QQ群：687364516  
> 最新的版本更新会在这里发布  
